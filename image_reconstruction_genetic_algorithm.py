...
img = PIL.Image.open('einstein.png').convert('RGB').resize((WIDTH, HEIGHT))
CELL_SIZE = 15
...
class Cromossomo():
    def __init__(self, ):
        self.genoma = [[celula() for _ in range(GRID_WIDTH)] for _ in range(GRID_HEIGHT)]
...
class GeneticAlgo():
    ...
    def selection(self):
        ...
        for _ in range(len(self.population)//2):
            n = min(3, len(self.population))
            candidatos = random.sample(self.population, n)

            melhor = max(candidatos, key=self.fitness)
            pais.append(melhor)
        return pais
